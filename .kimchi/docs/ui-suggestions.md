...
## 10. Dismissable Terminal History

**Current**: `Input` history stores everything in `self._history` (unlimited).

**Change**: Add `↑` to show history, `↓` to scroll, `á` key to clear. Limit to 100.

Already partially implemented in `terminal_widget.py`. Add:
...
def on_input_submitted(self, event: Input.Submitted) -> None:
    ...
    if cmd:
        ...
        self._history = self._history[-MAX_HISTORY:]  # trim
    self._hist_idx = -1
    self._write_pty(cmd + "\n")
```

Plus add a visual `×` clear button:
...
```python
# In TerminalWidget.compose()
yield Input(id="term-input", placeholder="❯")
yield Static("×", id="term-input-clear")

def on_term_input_clear_clicked(self) -> None:
    self._history.clear()
    self._hist_idx = -1
```

**Verification**: Type 101 commands → oldest is dropped. Click `×` → history cleared.